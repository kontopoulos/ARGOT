...
class nFoldCrossValidation(val sc: SparkContext, val numPartitions: Int, val numFold: Int) extends Experiment {
   ...
   */
  private def svmFoldValidation(currentFold: Int, files1: Array[String], files2: Array[String]): Map[String, Double] = {
    println("Separating training and testing datasets...")
    //get training and testing datasets from first category
    val testing1 = files1.slice(currentFold, currentFold+files1.size/numFold)
    val training1 = files1.slice(0, currentFold) ++ files1.slice(currentFold+files1.size/numFold, files1.size)
    //get training and testing datasets from second category
    val testing2 = files2.slice(currentFold, currentFold+files2.size/numFold)
    ...
    e2.readDataStringFromFile(training1(1))
    val g2 = nggc.getGraph(e2)
    var classGraph1 = m.getResult(g1, g2)
    for (i <- 2 to training1.size-1) {
    ...
    e3.readDataStringFromFile(training2(0))
    val g3 = nggc.getGraph(e3)
    val e4 = new StringEntity
    e4.readDataStringFromFile(training2(1))
   ...
   * @return map with evaluation metrics
   */
  private def naiveBayesFoldValidation(currentFold: Int, files1: Array[String], files2: Array[String]): Map[String, Double] = {
    println("Separating training and testing datasets...")
    //get training and testing datasets from first category
    val testing1 = files1.slice(currentFold, currentFold+files1.size/numFold)
    val training1 = files1.slice(0, currentFold) ++ files1.slice(currentFold+files1.size/numFold, files1.size)
  ...
  private def simpleFoldValidation(currentFold: Int, files1: Array[String], files2: Array[String]): Map[String, Double] = {
    ...
    var fn02 = 0
    testing1.foreach{ f =>
      val label = cls.test(f, List(g01, g02))
      if (label(0) == "C01") {
