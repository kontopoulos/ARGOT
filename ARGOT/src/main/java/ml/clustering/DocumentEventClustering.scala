...
class DocumentEventClustering(sc: SparkContext, numPartitions: Int) extends Clustering {
  ...
  // cartesian, filterNot, filterNot, groupBy, map, distinct
  // map to ids, subtract
  ...
  /*{
    ...
    documents.foreach{doc =>
      ...
      if (clustered.isEmpty) {
        ...
        val curE = new DistributedStringEntity
        curE.fromFile(sc, doc, numPartitions)
        val curG = wggc.getGraph(curE,numPartitions)
        ...
        for (i <- next to documents.length-1) {
          ...
          if (clustered.isEmpty) {
            ...
            val g = wggc.getGraph(e,numPartitions)
