...
import ml.clustering.{DocumentEventClustering, MatrixMCL}
...
class NGGSummarizer(val sc: SparkContext, val numPartitions: Int) extends MultiDocumentSummarizer {
  ...
  /*{
    ...
    var sentences = Array.empty[DocumentAtom]
    ...
    documents.foreach{d =>
      ...
      val s = ss.getSentences(e).asInstanceOf[RDD[DocumentAtom]]
  ...
  /**
    ...
    */
  private def getSimilarityMatrix(indexedAtoms: RDD[(DocumentAtom, Long)]): IndexedRowMatrix = ???
  /*{
    //number of sentences
