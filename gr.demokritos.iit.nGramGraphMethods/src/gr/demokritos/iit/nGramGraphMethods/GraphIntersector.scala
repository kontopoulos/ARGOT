 ...
 */
class GraphIntersector(val l: Double) extends BinaryGraphOperator with Serializable {
  ...
  def getResult(g1: Graph[String, Double], g2: Graph[String, Double]): Graph[String, Double] = {
    ...
    //combine edges
    val newEdges = pairs1.join(pairs2).map{ case ((srcId, dstId), (value1, value2)) => Edge(srcId, dstId, averageValues(value1, value2)) }
    //combine vertices
    val newVertices = g1.vertices.join(g2.vertices).map{ case(id, (name1, name2)) => (id, name1) }
    //create new graph
