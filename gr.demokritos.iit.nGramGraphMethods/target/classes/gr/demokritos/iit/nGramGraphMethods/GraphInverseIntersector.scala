...
class GraphInverseIntersector extends BinaryGraphOperator with Serializable {
  ...
  override def getResult(g1: Graph[String, Double], g2: Graph[String, Double]): Graph[String, Double] = {
    ...
    val emptyEdges1 = pairs1.subtract(pairs2).map{ case (srcId, dstId) => ((srcId, dstId),  0.0) }
    //get the edges from second graph that do not exist in the first graph
    val emptyEdges2 = pairs2.subtract(pairs1).map{ case (srcId, dstId) => ((srcId, dstId),  0.0) }
    ...
    val edges1 = valuedRDD1.join(emptyEdges1).map{ case ((srcId, dstId), (value1, value2)) => Edge(srcId, dstId, value1) }
    val edges2 = valuedRDD2.join(emptyEdges2).map{ case ((srcId, dstId), (value1, value2)) => Edge(srcId, dstId, value1) }
    //union edges
