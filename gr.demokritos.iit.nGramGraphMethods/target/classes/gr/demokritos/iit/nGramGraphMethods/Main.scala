...
object Main {
  def main(args: Array[String]) {
    ...
    val pairs1 = ngg.edges.map(edgeToPair)
    val pairs2 = ngg2.edges.map(edgeToPair)

    val test = pairs1.subtract(pairs2).map{ case (srcId, dstId) => ((srcId, dstId),  0.0) }
    val test1 = pairs2.subtract(pairs1).map{ case (srcId, dstId) => ((srcId, dstId),  0.0) }


    val values1 = ngg.edges.map(edgeAttrToPair)
    val values2 = ngg2.edges.map(edgeAttrToPair)

    val edges1 = values1.join(test).map{ case ((srcId, dstId), (value1, value2)) => Edge(srcId, dstId, value1) }
    val edges2 = values2.join(test1).map{ case ((srcId, dstId), (value1, value2)) => Edge(srcId, dstId, value1) }
    val edges = edges1.union(edges2)

    edges.distinct.collect.foreach(println)
    println(edges.distinct.count)*/

    //ngg.vertices.join(ngg2.vertices).map{ case(id, (name1, name2)) => (id, name1) }
    ...
    /*val textFile = sc.textFile("text1.txt")
    ...
    val textFile1 = sc.textFile("text2.txt")
    val counts1 = textFile1.flatMap(line => line.split(" "))
      .map(word => (word, 1))
      .reduceByKey(_ + _)
    val errors1 = textFile1.filter(line => line.contains("ERROR"))
    errors1.count()
    errors1.filter(line => line.contains("MySQL")).count()
    errors1.filter(line => line.contains("MySQL")).collect()*/
