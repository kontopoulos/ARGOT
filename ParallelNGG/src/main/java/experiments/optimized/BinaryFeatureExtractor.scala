...
class BinaryFeatureExtractor(sc: SparkContext, numPartitions: Int) {
  ...
  def train(classGraphs: Array[DistributedCachedNGramGraph], graphs: Array[(Double,NGramGraph)]) = {
  ...
  def test(model: ClassificationModel, classGraphs: Array[DistributedCachedNGramGraph], graphs: Array[(Double,NGramGraph)]) = {
